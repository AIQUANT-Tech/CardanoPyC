...
public class HaskellCompletionContributorTest extends BasePlatformTestCase {
    ...
    public void testPlutusIdentifierCompletions() {
        myFixture.configureByText("Test.hs", "txSignedBy");
        myFixture.complete(CompletionType.BASIC);
        List<String> completions = myFixture.getLookupElementStrings();

        assertNotNull("Completions should not be null", completions);
        assertTrue("Completions should contain 'txSignedBy'", completions.contains("txSignedBy"));
        assertTrue("Completions should contain 'Validator'", completions.contains("Validator"));
    }
